...
#define content_TextFond [UIFont systemFontOfSize:13]
...
static CGFloat const margin_X = 15;
...
- (instancetype)initWithTitle:(NSString *)title content:(NSString *)content delegate:(id<SGActionSheetDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitleArray:(NSArray *)otherButtonTitleArray{
    if (self = [super init]) {
...
- (void)setupSubviews2 {
    ...
    _message_label.textColor=UIColorFromRGB(0x333333);
    
    self.content_label = [[UILabel alloc] init];
    _content_label.textAlignment = NSTextAlignmentCenter;
    _content_label.numberOfLines = 0;
    _content_label.text = self.content;
    _content_label.font = content_TextFond;
    _content_label.textColor=UIColorFromRGB(0x333333);
    ...
    CGSize content_labelSize = [self sizeWithText:_content_label.text font:content_TextFond maxSize:CGSizeMake(message_label_W, MAXFLOAT)];
    
    if (_message_label.text) {
        
        // 设置Message的frame
        _message_label.frame = CGRectMake(0, margin_Y, SG_screenWidth, message_labelSize.height);
        _content_label.frame = CGRectMake(margin_X, margin_Y+message_labelSize.height+12, SG_screenWidth - 2 * margin_X, content_labelSize.height);

        
        
        
        // 创建Message背景视图
        CGFloat message_bgViewHeight = message_labelSize.height+12+content_labelSize.height + margin_Y *2;
        ...
        [bgView addSubview:_content_label];
...
//        _tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        ...
        self.sheetView.backgroundColor=UIColorFromRGB(0xf0f0f0);
        _cancelButton.layer.borderWidth=0.5;
        _cancelButton.layer.borderColor=UIColorFromRGB(0xd5d5d5).CGColor;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate_SG respondsToSelector:@selector(SGActionSheetdidSelectRowAtIndexPath:)]) {
        [self.delegate_SG SGActionSheetdidSelectRowAtIndexPath:indexPath.row];
    }
    [self dismiss];
}
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath//去除15像素点
{
