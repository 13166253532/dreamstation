...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self newLeftBarButton];
}
- (void)newLeftBarButton{
    
    UIBarButtonItem *newitem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"Arrow"] style:UIBarButtonItemStylePlain target:self action:@selector(pop)];
...
-(void)webView:(WKWebView *)webView didFailProvisionalNavigation:(WKNavigation *)navigation withError:(NSError *)error{
    [SMToastView showMessageCenter:self.view withMessage:@"视频播放失败，请稍后再试"];
    [self performSelector:@selector(goBackVC) withObject:nil afterDelay:2];
